  ...
  setProductForBuy(prevState => {    
    ...
    if (existingColorIndex !== -1) {
      ...
      updatedColorOfQuentity = prevState.colorOfQuentity.map((item, index) => 
        ...
        ? { ...item, colorQuentity: currentColorQuentity } 
        : item
      );
    } else {
      // Add a new object
...
//add to cart<>  
  const addtoCart = () => {
    localStorage.setItem("readyForCheckout",JSON.stringify(productForBuy));
    // trackAddToCart({
  ...
  return (
    <div className='ProductPageMain'>
        ...
        <div className='productSelection'>
            ...
            <Link to={`/checkout/`} onClick={addtoCart} style={{textDecoration:'none'}}><button className='buyNowbtntwo' disabled={true} >Buy Now</button></Link>
            </div>
        </div>
